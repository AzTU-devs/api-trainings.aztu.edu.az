...
public class CoursePublicController {
    ...
    @GetMapping
    @Operation(summary = "Browse published courses, optionally filtered by type and/or category", security = {})
    public ApiResponse<PageResponse<CourseSummaryDto>> browse(
            @RequestParam(required = false) CourseType type,
            @RequestParam(required = false) UUID categoryId,
            Pageable pageable) {
        return ApiResponse.ok(PageResponse.of(service.browsePublished(type, categoryId, pageable), mapper::toSummaryDto));
    }
